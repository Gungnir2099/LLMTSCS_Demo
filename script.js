...
function infoAppend(msg) {
    ...
    try {
        ...
        console.error("解析错误,预计 responses 文件格式问题：", e);
        return;
    }
    ...
    function displayMessage() {
        ...
        currentMsgIndex++;
        setTimeout(displayMessage, 2030);  // 设置下一条消息的显示时间
    }
...
function newStart() {
    ...
    let responsesUrl = 'readFile/responses.json'
                        ...
                        uploadFile(null, null, function () {
                            $.ajax({
                                ...
                                success: function (textData) {
                                    replayData.push(textData)
                                    textObject = {
                                        name: 'readFile/roadnet_1_1.json-flow_main_stream.json-LLMTLCSPPOFineTuneAFT-llama_traj_lora_13b-4_Phases-replayLogFile.txt'
                                    }
                                    uploadFile(textData, textObject, after_update)
                                }
                            });
                        })
                    }
...
document.getElementById("start-btn").addEventListener("click", newStart);
